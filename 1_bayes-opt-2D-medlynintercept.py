...
#setup case
CASE_NAME = 'hillslope-bayes-opt-2D-medlynintercept'
...
def blackbox_clm(slope_scalar, intercept_scalar):
    ...
    target_param_file = '/glade/u/home/marielj/cesm-hillslope/calib-surf-files/clm50_params_2D_bayesopt_medlynintercept.c240105.nc'
    base_grass_ms = 5.25
    base_grass_mi = 100

    #reset other pft params to original value
    change_pft_param('medlynslope', 2, 2.3499999, target_param_file) #conif
    change_pft_param('medlynslope', 8, 4.44999981, target_param_file) #decid

    #change grass parameters 
    change_pft_param('medlynslope', 13, slope_scalar*base_grass_ms, target_param_file)
    change_pft_param('medlynintercept', 13, intercept_scalar*base_grass_mi, target_param_file) 
    ...
    coldata = xr.open_mfdataset(CASE_NAME + '.clm2.h3.2015-01-01-00000.nc')
    data = xr.open_mfdataset(CASE_NAME + '.clm2.h2.2015-01-01-00000.nc')
...
clm_optimizer = BayesianOptimization(f = blackbox_clm, 
                                    pbounds = {'slope_scalar' : (1, 20), 
                                               'intercept_scalar' : (0, 3)
                                              }, 
                                    random_state = 75832, 
                                    verbose = 0
                                    )
...
logger = JSONLogger(path="/glade/u/home/marielj/cesm-hillslope/logs/hillslope_logs_2D_medlynintercept.json", reset = False)
