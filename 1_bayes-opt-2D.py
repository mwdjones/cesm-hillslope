...
def blackbox_clm(grass_scalar, tree_scalar):
    ...
    change_pft_param('medlynslope', 13, grass_scalar*base_grass_ms, target_param_file)

    #change tree parameters
    change_pft_param('medlynslope', 2, tree_scalar*base_conif_ms, target_param_file) #conif
    change_pft_param('medlynslope', 8, tree_scalar*base_decid_ms, target_param_file) #decid
...
logger = JSONLogger(path="/glade/u/home/marielj/cesm-hillslope/logs/hillslope_logs_2D_medlynslope.json", reset = False)
clm_optimizer.subscribe(Events.OPTIMIZATION_STEP, logger)

#update bounds range - go a little higher on both scalars
clm_optimizer.set_bounds(new_bounds={"grass_scalar": (1, 40), 
                                    "tree_scalar" : (1, 40)})
...
'''RUN OPTIMIZATION'''
clm_optimizer.maximize(init_points = 0, n_iter = 20,
